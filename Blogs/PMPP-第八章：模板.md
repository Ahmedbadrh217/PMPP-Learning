...
第七章学了卷积，这章学模板（Stencil）。你可能会问：这俩不是差不多吗？确实很像，但有微妙区别。**卷积**强调的是信号处理中的滑动加权和，权重来自滤波器；**模板**强调的是科学计算中的邻域更新，比如 PDE 求解、物理仿真。从 CUDA 优化角度看，技术是相通的，但模板计算有自己的特点：通常是**多次迭代**的，需要处理**时间步进**和**数据交换**。
...
模板计算（Stencil Computation）：用**固定的邻域模式**更新网格中的每个点。
...
书中推荐**策略 1**，因为加载更简单，且边界线程虽不计算但仍能并行执行。
...
**关键**：不能原地更新！读取邻居时可能读到已更新的值，导致错误。所以需要**双缓冲**：读旧写新，然后交换。
