...
前几章学了归约、排序等"规则"数据的并行算法。这章学**稀疏矩阵**——大部分元素为零的矩阵。稀疏矩阵在科学计算、图算法、机器学习中无处不在。存储所有零元素既浪费空间又浪费计算，所以需要特殊的存储格式和算法。第十四章讲解常见的稀疏格式和它们在 GPU 上的实现。
...
- [NVIDIA cuSPARSE Library](https://docs.nvidia.com/cuda/cusparse/)
