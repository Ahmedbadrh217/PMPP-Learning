...
第十一章学了前缀和，这章学**归并（Merge）**——把两个有序数组合并成一个有序数组。归并是排序算法的核心组件（归并排序），也是数据库操作的基础（JOIN）。并行归并的挑战在于：**输出位置取决于两个数组的数据内容**，不像矩阵乘法那样位置固定。第十二章讲解如何用**协同排名（Co-Rank）**技术解决这个"动态输入数据识别"问题。
