...
前两章学了归约和归并，这章学**排序**——把无序数据变成有序数据。排序是最基础的算法之一，几乎所有领域都用得到。GPU 排序的挑战在于：传统排序算法（如快速排序）的递归和数据依赖不适合 GPU 的 SIMT 模型。第十三章介绍适合 GPU 的排序算法，重点是**基数排序（Radix Sort）**——利用前缀和实现高效并行排序。
...
**关键**：每一轮排序必须是**稳定**的，保持相同键值元素的相对顺序。
...
阶段 3 的 `atomicAdd` 是瓶颈。可以用**本地排序 + 前缀和**替代：
...
- [NVIDIA CUB Library - Radix Sort](https://nvlabs.github.io/cub/structcub_1_1_device_radix_sort.html)
