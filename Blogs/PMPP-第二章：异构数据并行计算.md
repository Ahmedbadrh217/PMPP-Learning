...
上一章介绍了并行计算的发展历程和CPU、GPU的架构差异，算是给并行编程打了个底。第二章开始动真格了——直接上手写CUDA代码。虽然例子是经典的向量加法，但别小看它，这个简单的例子把CUDA编程的核心概念都涵盖了：内存管理、kernel函数、线程组织。整理这篇博客的过程中，我对异构编程的理解又深了一层。

> **📦 配套资源**：本文对应的完整代码实现在 [GitHub 仓库](https://github.com/psmarter/PMPP-Learning) 的 `Exercises/Chapter02/Exercise01` 中。代码包含向量乘法的 CUDA 实现、完整的测试框架（正确性验证 + 性能测试）以及详细的中文注释。所有代码都可以直接编译运行。
