...
第五章学了内存层次和 Tiling 技术，矩阵乘法性能提升了 10 倍。但这只是开始——实际优化中还有很多细节会影响性能。第六章系统梳理这些性能因素：内存合并、分支发散、资源分配、指令混合等。这章内容偏"工程实践"，掌握这些技巧，能让代码再快一倍。
...
```cuda
// 同一 warp 的线程，row 相同，col 连续
int idx = row * width + threadIdx.x;
float val = M[idx];  // 连续地址，合并
```

**按列遍历（不合并）**：

```cuda
// 同一 warp 的线程，col 相同，row 连续
int idx = threadIdx.x * width + col;
float val = M[idx];  // 步长 = width，不合并
```

如果 width = 1024，步长就是 1024×4 = 4096 字节，远超段大小，每个线程访问不同段。
